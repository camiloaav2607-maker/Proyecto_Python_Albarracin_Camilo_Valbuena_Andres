...
def login():
    data = cargar_datos()
    print("\n===== CAMPUSLANDS LOGIN =====")
    print("presione enter si quiere ingresar como camper, trainer o coordinador")
    reportes=input("presione 1 si quiere crear reporte de rutas:")
    if reportes == "1":
        trainers = listar_trainers()
        if trainers:
            print("\n--- TODOS LOS TRAINERS ---")
            for t in trainers: print(f"{t['nombres']} {t['apellidos']} | Rutas: {', '.join(t['rutas'])}")
